...
public class ca220454_WatchlistsOperations implements WatchlistsOperations {
    ...
    public boolean addMovieToWatchlist(Integer userId, Integer movieId) {
        // Ako film već postoji u watchlisti, ne dodaj ponovo
        ...
        try (PreparedStatement ps = conn.prepareStatement(query)) {
            ...
            // Ako dođe do greške (npr. FK violation), samo vrati false
            return false;
        }
    }
    ...
    public boolean isMovieInWatchlist(Integer userId, Integer movieId) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(query)) {
            ...
            ps.setInt(2, movieId);
            try (ResultSet rs = ps.executeQuery()) {
                return rs.next(); // Ako postoji bar jedan red, film je u listi
            }
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
    ...
    public boolean removeMovieFromWatchlist(Integer userId, Integer movieId) {
        ...
        try (PreparedStatement ps = conn.prepareStatement(query)) {
            ...
            ps.setInt(2, movieId);
            int rows = ps.executeUpdate();
            return rows > 0; // true ako je nešto obrisano
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
        }
